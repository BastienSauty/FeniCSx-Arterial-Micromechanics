...
os.environ["CC"] = "gcc"
# This tells FEniCS to be more patient with locks
os.environ["FENICS_JIT_TIMEOUT"] = "300" 
# Ensure workers don't try to use multiple threads internally
...
def run_single_tau(args):
    tau_val, name_simu_base, folder_name, simu_card, adventitia_card, media_card = args
    
    # Standard imports inside worker
    import os
    import pickle
    import copy
    from ChV_MultiscaleActiveStressRegulation.main_Vasoconstriction_25_11_24 import run_simulation

    tau_namefile = f"{name_simu_base}_{tau_val:.6f}_scal.pkl"
    tau_filepath = os.path.join("./outputs", folder_name, tau_namefile)
    
    # 1. Check for existing results
    if os.path.exists(tau_filepath):
        print(f"    [WORKER] Found existing file: tau={tau_val:.4f} -> Loading.")
        with open(tau_filepath, "rb") as f:
    ...
    # 3. Execution with status prints
    print(f"    [WORKER] STARTING simulation: tau={tau_val:.4f} (File: {tau_namefile})")
    ...
    try:
        ...
        print(f"    [WORKER] FINISHED simulation: tau={tau_val:.4f}")
        return result

    except Exception as e:
        print(f"    [WORKER ERROR] tau={tau_val:.4f} failed with error: {e}")
        raise e
...
def pipeline_SA(p, name, folder_name, passive_simu_card, simu_card, media_card, adventitia_card, index_passive):
    ...
    if passive_exists:
        ...
        # Check missing active points
        missing_count = sum(1 for tau in tau_list if not os.path.exists(f"./outputs/{folder_name}/{name_simu_base}_{tau:.6f}_scal.pkl"))
        if missing_count > 0:
            print(f"    [!] {missing_count} active simulations are missing.")
            needs_simulation = True

    # 3. EXECUTION
    if needs_simulation:
        # A. Warm-up (triggers JIT in main thread)
        # Note: We use the original cards here; they will get 'polluted' with C++ objects
        print(f"--> [WARM-UP] Initializing JIT dry-run for {name_simu_base}...")
        warmup_media = copy.deepcopy(media_card)
        warmup_media["cells"]["basal stress"] = 0.1
        run_simulation(f"{name_simu_base}_jit_warmup", folder_name, 
                       passive_simu_card, adventitia_card, warmup_media, dry_run=True)
        
        if not passive_exists:
             print(f"--> [RUNNING] Full Passive Simulation...")
             passive_result = run_simulation(f"{name_simu_base}_passive", folder_name, 
                                            passive_simu_card, adventitia_card, warmup_media)
             tau_list, tau_b_ref = calculate_tau_list(passive_result, index_passive)

        # B. Dispatch Parallel Pool with PRISTINE cards
        print(f"--> [PARALLEL] Dispatching {len(tau_list)} simulations...")
        
        # We pass the pristine copies captured at the start
        tasks = [(tau, name_simu_base, folder_name, pristine_simu, pristine_adv, pristine_media) 
                 for tau in tau_list]
        
        with ProcessPoolExecutor(max_workers=3) as executor:
            SA_results = list(executor.map(run_single_tau, tasks))
    else:
        # Loading from disk...
        SA_results = [pickle.load(open(f"./outputs/{folder_name}/{name_simu_base}_{tau:.6f}_scal.pkl", "rb")) for tau in tau_list]
