...
# LIVE symbolic UFL expressions (functions of self.un), re-derived from
# scratch every time they appear in a downstream form. At high fiber count N,
# dS = dot(Finv, dot(dtau, Finv.T)) and dSj = dot(Finv, dot(dtauj, Finv.T))
# dot a non-trivial expression (Finv = inv(Identity + grad(un)), itself a
# closed-form matrix inverse - not just a coefficient) into dtau/dtauj, which
# embed the O(N)-term sum from homogenization_scheme(). If FFCx does not
# fully factor out the shared Finv subexpression across that N-term sum, its
# own internal complexity gets re-expanded once per term, compounding the
# N-scaling - this was empirically localized (2026-08-17) as a RAM peak
# right at mat.dSj_expr's compile and again in build_solver()'s Jacobian
# compile (Jac = ufl.derivative(self.dF_res, self.du)), which goes through
# the same dS/incr pathway.
#
# Fn/J/Finv/B/tau depend ONLY on self.un and self.Sn (both plain state
# Functions - never on du/duj/v, the trial/test/direction functions Newton's
# ufl.derivative() actually differentiates against), so replacing them with
# a numeric snapshot (fem.Expression + .interpolate(), refreshed every time
# un/Sn change - same pattern already used for H2/M via inverse_matrix6_6)
# changes NOTHING about correctness: no derivative w.r.t. un is ever taken,
# and everywhere Fn/Finv/J appear alongside du/duj/v (dE, l, lj, the
# Neumann_follower BC's Fndu = Fn + grad_axi(du)) they're combined additively
# or as a fixed left/right factor, never differentiated themselves. See the
# project notes doc (claude/fiberdiscretization_SA_persistent_notes.md,
...
class Mechanical_Problem_axi:
    ...
    def build_weak_form(self):
        ...
        # Kinematic
        Fn_sym = ufl.variable(ufl.Identity(3)+self.grad_axi(self.un))
        J_sym = ufl.det(Fn_sym)
        Finv_sym = ufl.inv(Fn_sym)
        B_sym = ufl.dot(Fn_sym, Fn_sym.T) # left Cauchy-Green

        if MATERIALIZE_KINEMATICS:
            # Materialize Fn/J/Finv/B into numeric snapshots instead of leaving
            # them as live symbolic expressions of self.un - see the
            # MATERIALIZE_KINEMATICS docstring above this class for why this is
            # safe (no derivative is ever taken w.r.t. un) and what problem it
            # fixes. self._Fn_expr etc. are kept around so update_kinematics()
            # (called from update_local_quantities()) can re-interpolate them
            # every time self.un/self.Sn change.
            self._Fn_expr = fem.Expression(Fn_sym, self.V_mat.element.interpolation_points())
            self.Fn = fem.Function(self.V_mat)
            self.Fn.interpolate(self._Fn_expr)

            self._J_expr = fem.Expression(J_sym, self.V_scalar.element.interpolation_points())
            self.J = fem.Function(self.V_scalar)
            self.J.interpolate(self._J_expr)

            self._Finv_expr = fem.Expression(Finv_sym, self.V_mat.element.interpolation_points())
            self.Finv = fem.Function(self.V_mat)
            self.Finv.interpolate(self._Finv_expr)

            self._B_expr = fem.Expression(B_sym, self.V_mat.element.interpolation_points())
            self.B = fem.Function(self.V_mat)
            self.B.interpolate(self._B_expr)
        else:
            self.Fn = Fn_sym
            self.J = J_sym
            self.Finv = Finv_sym
        ...
        # materialized for the same reason as Fn/J/Finv/B above: it's dotted
        # with the O(N)-term dtau/dtauj sum a few lines below (into dS/dSj),
        # so leaving it symbolic risks the same N-way re-expansion.
        tau_sym = ufl.dot(self.Fn, ufl.dot(Voigt2Tensor(self.Sn), self.Fn.T)) # in global CS
        if MATERIALIZE_KINEMATICS:
            self._tau_expr = fem.Expression(tau_sym, self.V_mat.element.interpolation_points())
            self.tau = fem.Function(self.V_mat)
            self.tau.interpolate(self._tau_expr)
            tau = self.tau
        else:
            tau = tau_sym
    ...
    def update_kinematics(self):
        """
        Refresh the materialized (numeric) Fn/J/Finv/B/tau snapshots from the
        CURRENT self.un/self.Sn - a no-op if MATERIALIZE_KINEMATICS is False
        (self.Fn etc. are then still the original live symbolic expressions,
        which stay valid automatically). Must be called every time self.un or
        self.Sn changes, before the next residual/Jacobian assembly or
        mat.dSj_expr evaluation - update_local_quantities() already calls this
        at the right point (see compute_increment()), and build_weak_form()
        already populates the initial (un=0) values, so callers should not
        normally need to call this directly.
        """
        if not MATERIALIZE_KINEMATICS:
            return
        self.Fn.interpolate(self._Fn_expr)
        self.Fn.x.scatter_forward()
        self.J.interpolate(self._J_expr)
        self.J.x.scatter_forward()
        self.Finv.interpolate(self._Finv_expr)
        self.Finv.x.scatter_forward()
        self.B.interpolate(self._B_expr)
        self.B.x.scatter_forward()
        self.tau.interpolate(self._tau_expr)
        self.tau.x.scatter_forward()

    def update_local_quantities(self):
        """
        compute stiffness matrix. temporary solution
        """
        # Refresh Fn/J/Finv/B/tau (see MATERIALIZE_KINEMATICS) BEFORE the
        # per-inclusion/material updates below, since mat.dSj_expr (compiled
        # in build_weak_form(), evaluated via update_micro_mech()-adjacent
        # code in compute_increment()) and the next residual/Jacobian
        # assembly both need them to reflect the un/Sn state as of the END of
        # the previous compute_increment() sub-step.
